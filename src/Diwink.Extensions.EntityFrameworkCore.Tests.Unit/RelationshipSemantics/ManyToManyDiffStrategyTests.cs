...
public class ManyToManyDiffStrategyTests
{
    ...
    [Fact]
    public async Task Updates_existing_payload_association_fields_in_place()
    {
        // Arrange
        var dbName = Guid.NewGuid().ToString();
...

        {
            await using var seedCtx = CreateInMemoryContext(dbName);
            var catalog = new LearningCatalog { Id = catalogId, Name = "Cat" };
            var mentor = new Mentor { Id = mentorId, DisplayName = "M1", Status = "Active" };
            var course = new Course
            {
                Id = courseId,
                CatalogId = catalogId,
                ...
                [
                    new CourseMentorAssignment
                    {
                        ...
                        MentorId = mentorId,
                        Role = "Lead",
                        AssignedOnUtc = new DateTime(2026, 1, 1, 0, 0, 0, DateTimeKind.Utc),
                        AllocationPercent = 50m
                    }
                ]
            };
            ...
            var updated = new Course
            {
                ...
                [
                    new CourseMentorAssignment
                    {
                        CourseId = courseId,
                        MentorId = mentorId,
                        Role = "Senior Lead",      // changed
                        AssignedOnUtc = new DateTime(2026, 1, 1, 0, 0, 0, DateTimeKind.Utc),
                        AllocationPercent = 100m   // changed
                    }
                ]
            };
...

        {
            ...
            var assignment = await verifyCtx.CourseMentorAssignments
                .SingleAsync(a => a.CourseId == courseId && a.MentorId == mentorId);

            assignment.Role.Should().Be("Senior Lead");
            assignment.AllocationPercent.Should().Be(100m);

            // Mentor entity must not be deleted
            var mentorExists = await verifyCtx.Mentors.AnyAsync(m => m.Id == mentorId);
            mentorExists.Should().BeTrue("updating payload must not remove the related mentor");
        }
    }

    [Fact]
    public async Task Removing_all_skip_navigation_links_preserves_related_entities()
    {
        // Arrange
        await using var ctx = CreateInMemoryContext();
        var tag1 = new TopicTag { Id = Guid.NewGuid(), Label = "Tag1" };
        var tag2 = new TopicTag { Id = Guid.NewGuid(), Label = "Tag2" };
        var course = new Course
        {
            Id = Guid.NewGuid(),
            CatalogId = Guid.NewGuid(),
        ...
        // Assert
        var reloaded = await ctx.Courses
            .Include(c => c.Tags)
            .FirstAsync(c => c.Id == course.Id);
        reloaded.Tags.Should().BeEmpty("all links should be removed");

        var tagCount = await ctx.TopicTags.CountAsync();
        tagCount.Should().Be(2, "tag entities themselves must be preserved");
    }
}